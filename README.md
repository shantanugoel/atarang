...
There is no backend or companion service. Network access is used only to fetch the YouTube audio and to update the embedded `yt-dlp` extractor. Separation and playback happen entirely on the iPhone.
...
The project uses automatic signing with development team `A8L3M4746U`. The bundled FP16 model is about 233 MB before Xcode compilation, and processing is compute intensive; recent iPhones will work best. The first URL extraction also downloads the current `yt-dlp` Python module on the phone.
