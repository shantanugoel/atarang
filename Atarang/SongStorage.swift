...
struct SongStorage: Sendable, Equatable {
    ...
    static func applyBackupPolicy(to folder: URL, audioFilename: String) {
        excludeFromBackup(folder.appendingPathComponent(audioFilename))
    }

    /// The same rule for a separation folder.
    ///
    ...
    /// them. Anything else in the folder — a `practice.json` that fell back to
    /// living here because the original was deleted — is the user's own work
    /// and stays backed up.
    ...
    /// Re-applies the policy across the whole library at launch.
    ///
    /// The rule is enforced when an entry is published, but the rule has
    /// changed — separations were never excluded before this — and a folder
    /// written by an earlier build carries whatever flags it was given then.
    /// Cheap enough to redo every launch: it is a resource-value write per
    /// file, on folders the app has already had to enumerate.
